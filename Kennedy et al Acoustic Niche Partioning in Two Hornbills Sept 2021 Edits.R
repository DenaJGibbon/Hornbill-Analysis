...
# Model selection
## Model 1 and 2: Good for temporal comparison
# Argue these are a random set of locations where the recorders are; a lot of locations which means a lot of parameters
# Random sample, a lot of parameters, more parsimonious model
# Model1:
#   Y ~ Hour*Species + (1|Recorder/Species)
# 
# Model2:
#   Y ~ Hour + Species + (1|Recorder/Species)
# 
## Model 3 and 4: To get at spatial
# Model3:
#   Y ~ 1 + (1|Recorder/Species)
# 
# Model4: This model doesn't make sense; take out and do Model5
#   Y ~ 1 + (1|Recorder)

# Model5:
#   Y ~ Hour + Species + (1|Recorder)
...
Hour.mod.nested.recorder.null.fe <- # This is a nested random effect
  glmmTMB(Freq ~ Recorder+Call.type, family = "nbinom2", data=HornbillDataAggregateByDate)

anova(Hour.mod.nested.recorder.null.nest,Hour.mod.nested.recorder.null.fe)
emmeans(Hour.mod.nested.recorder.null.nest,pairwise ~ Call.type | Recorder)

summary(Hour.mod.nested.recorder.null.nest)
...
## The second two models are testing for temporal variation
...
# When comparing 2 and 4 there is a big difference (that reflects spatial partitioning)
Model3 <- 
  glmmTMB(Freq ~ Hour + Call.type +(1|Recorder) , family = "nbinom2", data=HornbillDataAggregateByDate)

Model4 <- # This is a nested random effect
  glmmTMB(Freq ~  (1|Recorder/Call.type), family = "nbinom2", data=HornbillDataAggregateByDate)

# Now we compare the models using AIC
AICctab(Model1,Model2,Model3,Model4,weights=T,
        base=T)
